<main class="content">
      ...
      <div class="dashItem">
        ...
        <div class="dashWidget dashautoHeight">
          <div class="notif-dash">
            ...
            <div style="color:black;font-size:22px; font-weight: bold;" align="center"><?= number_format($kurs,2,",",".") ?></div>
          </div>
        </div>
      </div>

      <!-- PPA Revisi -->
      <?php if($this->session->userdata('group')=='101' and $revisi >=1){ ?>
      <div class="dashItem">
        <h2 class="dashHead" align="center"><i class="fa-regular fa-bell" style="color:#ff7315;"></i> PPA Revisi</h2>
        <a style="text-decoration:none" href="#" onclick="addTab('<?php echo base_url("ppa_koreksi")?>','Daftar Koreksi PPA')">
          <div class="dashWidget autoHeight">
            <div class="notif-dash">
              <div style="color: #ff7315; font-size:32px; font-weight: bold;" align="center"><?php echo $revisi ?></div>
              <div style="color:black;font-size:14px;" align="center">Ada <?php echo $revisi ?> permohonan perlu revisi.</div>
              <div style="color: #999898ff;font-size:12px; font-style:italic" align="center">[Klik untuk melihat daftar!]</div>
            </div>
          </div>
        </a>
      </div>
  ...
  <?php 
    ...
    <?php if ($this->session->userdata('group') == '101' && isset($checkinv) && !empty($checkinv)) { ?>
    <div class="dashboard">
          ...
          <div class="notif-dash">
            ...
            <div align="center">
              <?php foreach($checkinv as $invoice): 
                echo 'Nomor Invoice: 
                      <span style="color: color: #ff7315; font-size:14px; font-weight: bold;">'.$invoice['NO_INVOICE'].
                      '</span>  Tanggal: '.$invoice['TANGGAL_INVOICE'].' Total:  
                      <span style="color: color: #ff7315; font-size:16px; font-weight: bold;">'.number_format($invoice['TOTAL_USD'], 0, ',', '.').'
                      </span> Sisa:  <span style="color: color: #ff7315; font-size:16px; font-weight: bold;">'.number_format($invoice['SISA'], 0, ',', '.').'</span>'?>
